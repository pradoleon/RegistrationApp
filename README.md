...
> as you know the database context gets the connection string from the end layer. in this project, our context gets the database connection string from the *Api* layer. so we need to define the database connection string in the file *appsettings.json* in the *Api* layer. like this:
  `"ConnectionStrings": {
    "RegistrationAppDbContext": "Server=.;Database=RegistrationApp;Trusted_Connection=True;MultipleActiveResultSets=true"
  }`
...
- create a folder with the name *RegistrationApp.UI*.
- run powershell with *administrator* mode and route to the *RegistrationApp.UI* path then execute the command `ng new UI`.
...
- I use **Visual Studio Code** to develop my angular app, so open the VS Code go to *File >>> Open Folder* then route to *PersonRegistrationApp.UI/UI* and click the *Select Folder* button, then you can see your angular app in the VS Code.
